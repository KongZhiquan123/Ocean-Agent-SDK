/**
 * @file workflow.ts
 * @description 训练工作流 - 4 阶段分步确认逻辑
 *              函数式实现，替代原 TrainingWorkflow 类。
 *              Token 机制：SHA-256 签名覆盖关键字段，参数变更后 token 失效。
 *
 * @author Leizheng
 ...
 *
 * @changelog
 *   - 2026-03-12 kongzhiquan: v3.1.0 新增 missing_token 分支
 ...
 *   - 2026-03-12 kongzhiquan: v3.0.0 重构为函数式，移除 TrainingWorkflow 类
 *     - 暴露 mergeParams() + resolveStage() 作为唯一公开 API
 *     - TrainingState 常量对象改为联合类型 TrainingStateType
 *     - 阶段判断和提示构建内聚在同一文件
...
const TOKEN_SALT = 'ocean-SR-training-v2'
...
export interface SrTrainingWorkflowParams {
  // ====== 阶段1: 数据确认 ======
  dataset_root?: string
  log_dir?: string
...
export interface SrDatasetInfo {
  status: string
  dataset_root: string
  dyn_vars: string[]
  scale: number | null
...
function getModelDivisor(modelName?: string): number {
  ...
  if (['DDPM', 'SR3', 'MG-DDPM', 'ReMiG'].includes(modelName)) return 32
...
function computeAutoPatchSize(
  params: SrTrainingWorkflowParams,
...
function resolveUseAmp(params: SrTrainingWorkflowParams): boolean {
...
function formatPatchStrategy(params: SrTrainingWorkflowParams): string {
  if (params.patch_size !== undefined && params.patch_size !== null) {
...
function hasDataParams(p: SrTrainingWorkflowParams): boolean {
  return !!(p.dataset_root && p.log_dir)
}

function hasModelParam(p: SrTrainingWorkflowParams): boolean {
  return !!p.model_name
}

function hasTrainingParams(p: SrTrainingWorkflowParams): boolean {
  return !!(
    p.dyn_vars && p.dyn_vars.length > 0 &&
    p.scale !== undefined && p.scale > 0 &&
    p.epochs !== undefined && p.epochs > 0 &&
    p.lr !== undefined && p.lr > 0 &&
    p.batch_size !== undefined && p.batch_size > 0 &&
    p.device_ids && p.device_ids.length > 0
  )
}
...
/**
 * 生成 SHA-256 确认 Token。
 * use_amp 不参与签名：系统可能自动调整，不应因此导致 token 失效引发振荡。
 */
export function generateConfirmationToken(params: SrTrainingWorkflowParams): string {
  const tokenData = {
...
function validateConfirmationToken(params: SrTrainingWorkflowParams): boolean {
...
function buildModelSelectionPrompt(
  ...
  if (modelList && modelList.length > 0) {
    ...
    const formatGroup = (models: SrModelInfo[]) =>
      models
        .map(m => {
...
检测到跳步行为：
已设置 user_confirmed=true，但未提供 confirmation_token。
...
- device_ids: [${params.device_ids?.join(', ')}]
- distribute: ${params.distribute}
- use_amp: ${resolveUseAmp(params)}（不参与 Token 签名，系统可自动调整）
- gradient_checkpointing: ${params.gradient_checkpointing}
- patch_size: ${params.patch_size ?? '未设置'}
...
export function mergeParams(
  args: SrTrainingWorkflowParams,
  sessionOverrides?: SrTrainingWorkflowParams
): SrTrainingWorkflowParams {
 ...
 * @returns TrainingStagePromptResult（仍在某阶段）或 null（所有阶段通过，可继续执行）
 */
...
): SrTrainingStagePromptResult | null {
