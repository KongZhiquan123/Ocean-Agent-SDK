...
>
> **重要**：正常情况下训练必须通过 `ocean_forecast_train_start` 工具启动。
...
> **所有模板仅供参考**。正常流程应使用 `ocean_forecast_train_start` 工具。
...
/home/lz/miniconda3/envs/pytorch/bin/python \
  {log_dir}/_ocean_forecast_code/main.py \
  --mode train \
...
/home/lz/miniconda3/envs/pytorch/bin/python \
  ...
  --nproc_per_node=4 \
  --master_port=29500 \
  {log_dir}/_ocean_forecast_code/main_ddp.py \
  --mode train \
  --config {log_dir}/config.yaml
```
