...
│   → 根因：GPU 显存不足（forecast 模型输入张量 (B, H, W, in_t*C) 较大）
│   → 方案：减小 batch_size / 减小 in_t / 开启 AMP / 开启 gradient_checkpointing
│   → 参考：errors.md #CUDA OOM
│
...
│   → 根因：in_t/out_t 与 ckpt 不一致 / 模型通道数与 dyn_vars 不匹配
│   → 方案：续训时 in_t/out_t 必须与原始训练一致；确认 dyn_vars 数量正确
│
└── 错误含 "spatial shape" / "Cannot detect"
    → 根因：NPY 文件形状不一致或 var_names.json 配置错误
    → 方案：运行 ocean_forecast_train_start 第一阶段验证数据
```
...
├── 使用了错误的 Python 路径
│   → 恢复：停止手动尝试，改用 ocean_forecast_train_start 工具
│
├── 使用了废弃的 launcher
│   → 恢复：停止手动尝试，改用 ocean_forecast_train_start 工具
│
...
| 6 | `Shape mismatch` / `size mismatch` | 模型通道数与数据不匹配，或 ckpt 中 in_t/out_t 不一致 | 确认 dyn_vars、in_t、out_t 正确；续训不可更换模型架构或时序参数 |
...
| 10 | `RuntimeError: CUDA out of memory` | GPU 显存不足 | 按优先级：AMP → 减 batch_size → 减 in_t → gradient_checkpointing |

---

## 工作空间文件结构

训练工具会将 `scripts/ocean-forecast-training/` 复制到工作空间 `{log_dir}/_ocean_forecast_code/`。

### 目录树

```
{log_dir}/
├── _ocean_forecast_code/      ← 工作空间副本（可修改）
│   ├── main.py                ← 单卡/DP 入口
│   ├── main_ddp.py            ← DDP 入口
│   ├── config.py              ← 配置解析
│   ├── utils/                 ← 工具函数
...
│   │   ├── base/              ← 共享基础模块（forecast_adapter 等）
│   │   └── {model_name}/      ← 选中的模型代码
│   ├── trainers/              ← 训练器（base.py）
│   ├── forecastors/           ← 推理辅助模块
│   └── datasets/              ← 数据集加载
├── config.yaml                ← 训练配置
├── train.log                  ← 训练日志
...
| 目录/文件 | 可修改 | 说明 |
|-----------|--------|------|
| `{log_dir}/_ocean_forecast_code/**` | 是 | 工作空间副本，可自由修改调试 |
| `scripts/ocean-forecast-training/**` | **否** | 原始目录，禁止修改 |
