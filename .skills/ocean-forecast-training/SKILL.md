...
<!--
Changelog:
  - 2026-03-04 Leizheng: v1.2.0 新增 3 个运维工作流 + 禁止行为更新
    - 断点续训工作流：ckpt_path 续训、追加 epoch、微调 lr（额外约束 in_t/out_t 一致）
...
- **时序参数不可变**：续训时 `in_t` 和 `out_t` 必须与原始训练一致（影响模型输入输出维度）
...
### 代码修改规范
- **只改工作空间副本**：修改训练代码时只能编辑 `{log_dir}/_ocean_forecast_code/` 目录
- **禁止修改原始目录**：`scripts/ocean-forecast-training/` 目录禁止修改
